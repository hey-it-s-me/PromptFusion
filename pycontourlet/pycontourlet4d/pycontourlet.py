...
from pycontourlet.pycontourlet4d.dfilters import dfilters
from pycontourlet.pycontourlet4d.modulate2 import modulate2
import pyximport
import numpy as np

pyximport.install(setup_args={'include_dirs': np.get_include()})
from pycontourlet.pycontourlet4d.resamp4c import resamp4c
import torch
...
def lpdec(x, h, g, device=torch.device("cpu")):
    ...
    xlo[:, :, ::2, ::2] = c.to(device)
    x = x.to(device)
    tmp = sefilter2(xlo, g, g, 'per', adjust * torch.tensor([1, 1]), device=device)
    d = x - tmp

    return c.to(device), d
...
def resamp(x, type_, shift, extmod, device=torch.device("cpu")):
    ...
    if type_ == 0 or type_ == 1:
        ...
        y = torch.from_numpy(resamp4c(x.transpose(0, 1, 3, 2), type_ - 2, shift, extmod).transpose(0, 1, 3, 2)).to(
            device)
    else:
